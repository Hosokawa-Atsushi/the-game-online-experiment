                ...
                (t
                 (let ((p1 (previous-production-struct (compilation-module-previous module))))
                   (bt:with-recursive-lock-held ((production-lock p1))
                     (let ((p1-s (production-composition-rep p1))
                           (p2-s (production-composition-rep production)))
    ...
    (dolist (b p1-indices)
      (let* ((comp-type (get-compilation-type-struct (car b) module))
        ...
        (when instantiate
          ...
          (let* ((c2 (find-if (lambda (x) (and (char= (compose-rep-op x) #\=) (eq b (compose-rep-name x)))) (first p2-s)))
                 (a1= (find-if (lambda (x) (and (char= (compose-rep-op x) #\=) (eq b (compose-rep-name x)))) (second p1-s)))
...
(defun production-comparison-rep (production)
  (car (produce-standard-representation production)))

(defun production-composition-rep (production)
  (cdr (produce-standard-representation production)))
...
(defun produce-standard-representation (production) ;; Only works for composeable productions -- no mv-binds and no nested bind variables
  (aif (production-standard-rep production)
       it
       (let ((lhs (stable-sort (mapcar (lambda (x)
                                 ...
                                 (production-lhs production))
                               'sort-production-ops))
             (rhs (stable-sort (mapcar (lambda (x)
                                         (list (production-statement-op x)
                                               (production-statement-target x)
                                               (if (char= #\! (production-statement-op x))
                                                   (production-statement-definition x)
           ...
           (setf compose-rep (list (mapcar (lambda (x)
                                             (list (list 
                                                    ...
                                                    (case (first x)
                                                      ...
                                                      (#\? (intern (string-upcase (concatenate 'string "?" (symbol-name (second x)) ">"))))
                                                      (#\= (let ((s (intern (string-upcase (concatenate 'string "=" (symbol-name (second x)) ">"))))
                                                                 (v (intern (string-upcase (concatenate 'string "=" (symbol-name (second x)))))))
                                                             (pushnew v buffer-vars)
                                                             s))))
                                                   (third x)))
                                     lhs)
                                   (mapcar (lambda (x)
         ...
         (setf (production-standard-rep production) (cons (list lhs rhs) compose-rep)))))
