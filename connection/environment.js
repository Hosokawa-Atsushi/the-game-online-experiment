...
const actrPortnum = Number(process.argv[3]);
console.log('actrPortnum=' + actrPortnum);
const ifPortnum = Number(process.argv[4]);
console.log('ifPortnum=' + ifPortnum);
...
app.get('/expwindow', function(req,res) {
  res.sendFile(__dirname + '/expwindow.html');
});

// Start the http server on the given port.

http.listen(ifPortnum);
...
client.connect(actrPortnum,address)
...
client.on('close', (hadError) => {
  console.log('[environment] closed, hadError: ' + hadError);
});
client.on('timeout', () => {
  console.log('[environment] timeout');
});
...
function monitorOpponentCard() {
  intervalId = setInterval(() => {
    client.write(`{\"method\":\"evaluate\",\"params\":[\"refer-to-another-action\",false],\"id\":null}\u0004`);
  }, 1000);
}
        ...
        if (m.params[0] == "vv") { // it's the exp window command handled here
          ...
          switch (p[0]) {
            ...
            case "delay" :
              ...
              setTimeout(() => {
                client.write(`{\"method\":\"evaluate\",\"params\":[\"fix-model-action\",false],\"id\":null}\u0004`);
              }, p[1]*1000);
...
process.on('message', (another) => {
  console.log(`{\"method\":\"evaluate\",\"params\":[\"set-players\",false,\"${user}\",\"${another.isModel}\",\"${another.id}\"],\"id\":null}\u0004`);
  client.write(`{\"method\":\"evaluate\",\"params\":[\"set-players\",false,\"${user}\",\"${another.isModel}\",\"${another.id}\"],\"id\":null}\u0004`);
});
...
expio.on('connection',function(socket){
  ...
  socket.on('redraw', () => {
    console.log('再描画');
    console.log(`{\"method\":\"evaluate\",\"params\":[\"redraw\",false],\"id\":null}\u0004`);
    client.write(`{\"method\":\"evaluate\",\"params\":[\"redraw\",false],\"id\":null}\u0004`);
  });
